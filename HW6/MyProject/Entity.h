...
enum EntityType { PLATFORM, PLAYER, MINION, GATE, BULLET };
enum AIType { WALKER, RUNNER, SHOOTER, GUARD };
enum AIState { WALKING, IDLE, ATTACKING };
enum AITeam {RED, BLUE};
...
public:
    ...
    bool get_state() { return is_active; }
    ...
    int        const get_team()         const { return ai_team; };
    bool        const get_gated()       const { return gated; };
    ...
    void const set_hp(float new_hp) { hp = new_hp; };
    void const set_team(AITeam new_team) { ai_team = new_team; };
    void const set_gated(bool new_gated) { gated = new_gated; };
};